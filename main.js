...
var coloralpha = 0.5 // alpha to render elements in
var colorcap = 255 // maximum color brightness

var rotation = 1 / 10 // how much to rotate the canvas each frame
var clearScreenAlpha = 0.9 // alpha value for the screen erase each frame

var zoom = 1.5 // zoom level

var triheight = 0.1 // modifier for the triangle height (lower is shorter)
...
function draw() {
  ...
  if (canvas.getContext) {
    ...
    for (var i = 0; i < elements; i++) {
      if (getRandom(1, 5) == 1) {
        if (increments[i] !== 0) increments[i] = 0
        else increments[i] = getRandom(-1, 1) * colorstep
      }
